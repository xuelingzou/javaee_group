  ...
  <div>
    ...
    >
      ...
      <el-table-column
          ...
          label="状态"
          min-width="90"
          :filters="[{ text: '已接单', value: '已接单' }, { text: '已发货', value: '已发货' }, { text: '已到达', value: '已到达' }]"
...
export default {
  ...
  mounted() {
    ...
    loadData(){
      ...
      request.get('/api/order/Sys/v1/orders/coid/'+ query.id , //调取后端注册接口
      ).then((respond) => {
        // let res = respond
    ...
    send(index, row) {
      ...
      layer.confirm('是否确定发货？', function(index){
        //向服务端发送发货指令
        request.post(
            '/api/order/Sys/v1/orders/'+oid+'/state',
            {
              coid:query.id,
              state:'发货'
            },
